...
RUN_NAME = f"CLIP2_study_lr{LR}_valcoeff{VALUE_COEF}_clip{CLIP_EPS}_{datetime.now().strftime('%m%d%Y_%H%M%S')}"
...
torch.save(
    model.state_dict(),
    f"{CHECKPOINT_PATH}/CLIP2_study_lr{LR}_valcoeff{VALUE_COEF}_clip{CLIP_EPS}.pt"
)
